...
from main import app  # Asegúrate de que la importación de app sea correcta
...
def get_auth_token():
    ...
    rol_test = db.query(model_rol.Rol).filter(model_rol.Rol.nombre == "TEST_ROLE").first()  # Cambié NombreRol por nombre
    if not rol_test:
        rol_test = model_rol.Rol(nombre="TEST_ROLE", estado=True)  # Cambié NombreRol por nombre
        db.add(rol_test)
        db.commit()
        db.refresh(rol_test)
